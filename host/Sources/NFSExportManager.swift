...
final class NFSExportManager {
    ...
    func install(for mounts: [MountConfig]) throws {
        ...
        DVMLog.log(phase: .mounting, "NFS exports: guest=\(guestIP.rawValue) paths=\(paths.map(\.rawValue).joined(separator: ","))")
    ...
    @discardableResult
    private func run(_ executable: String, _ arguments: [String], allowNonZeroExit: Bool = false) throws
